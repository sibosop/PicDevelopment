...
UInt16	readEWord(UInt8 addr)
{
	UInt16 rval;
	rval = eeprom_read(addr);
	++addr;
	rval |= eeprom_read(addr) << 8;
	return rval;
}

void writeEWord(UInt8 addr, UInt16 val)
{
	eeprom_write(addr,(val & 0x00ff));
	++addr;
	eeprom_write(addr,val >> 8 );
}
...
{
	UInt8	i;
	UInt8	addr = 1;	//skip the write flag
	for ( i = 0; i < NUM_LINES; ++i, addr +=2 )
	{
		etest = readEWord(addr);
		etest1 = lineVals[i].maxLineTime;
		if ( etest != etest1 )
		{
			writeEWord(addr,etest1);
		}

		addr += 2;
		etest = readEWord(addr);
		etest1 = lineVals[i].minLineTime;
		if ( etest != etest1 )
		{
			writeEWord(addr,etest1);
		}
...
__EEPROM_DATA(0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd);
void
initLines(void)
{
	UInt8 i;
