...
motor1		res	3		; motor structure one	
motor2		res	3		; motor structure two
xorm		res	1		; xor mask
oscval		res	1		; "random" osc bit
...
SWTDBC1		EQU	1		; switch debounce flag
SWTDBC2		EQU	2		; switch debounce flag


SWT1		EQU	2		; portA switch bit
SWT2		EQU	3		; portA switch bit
...
; initialization code
		...
		clrf	state
loop	
		btfss	PORTA,SWT1	; is swt 1 pressed?
		goto	clearswt1	; no test switch 2
		btfsc	state,SWTDBC1	; check debounce l
		goto	testswt2	; debug not cleared, try switch 2
		bsf	state,SWTDBC1	; switch just pressed
		movlw	motor1
		addlw	MOTDEL
		...
		goto	testswt2
clearswt1	
		bcf	state,SWTDBC1	; clear the swtich debounce
testswt2
		btfss	PORTA,SWT2	; is switch two down
		goto	clearswt2	; no, clear the debounce
		btfsc	state,SWTDBC2	; yes, see if debounce set
		goto	spinosc		; debounce set, skip change of delay
		movlw	motor2		; set pointer to delay count
		addlw	MOTDEL
		movwf	FSR
		call	getdelay	; get a delay value
		movwf	INDF		; set the new delay
		goto	spinosc
clearswt2
		bcf	state,SWTDBC2	; clear the swtich debounce
spinosc
		rrf	oscval,f	; mangle delay value some more	
		movlw	0xfc		; clear bottom two bits for	
		andwf	oscval,f	; or with random osc
		movf	PORTA,w		; read the a input
		andlw	0x3		; mask out the non - osc bits
		iorwf	oscval,f
		btfss	state,ALARM	; test for interrupt alarm
		goto	loop		; loop if no alarm
		bcf	state,ALARM	; clear the alarm bit
		clrf	stepbuf		; clear the ouput value
		movlw	motor1		; get pointer to motor1
		movwf	motorptr	; save pointer for subroutine
		call	checkmotor	; determine step value
		movwf	stepbuf		; get step value for motor1
		movlw	motor2		; set pointer	
		movwf	motorptr	; to motor 2
		call	checkmotor	; check counts on motor2
		swapf	stepbuf,f	; pack both step values
		iorwf	stepbuf,w	; in the buffer for the 
		movwf	PORTB		; output port
...
initmotor	
		movlw	MOTDEL
		addwf	motorptr,w	; get pointer to motor structure
		movwf	FSR		; set to current delay count
		call	getdelay
		movwf	INDF		; delay count to 0 (it will be reset)
